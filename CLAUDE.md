...
style.css       716줄  커스텀 CSS (Tailwind CDN 위에 얹음)
core.js         853줄  동기화(REST), 상태(State), 탭, Escape, 유틸
tmdb.js         703줄  TMDB 검색/상세/OTT/추천·발굴/컬렉션 캐시
watchlog.js    2311줄  카드 목록, 필터, 등록·수정 모달, 별점 몰아넣기, 설정 탭
...
- **조회 → 미리보기 → 적용** 세 단계다(2026-08-07). 조회 단계는 `State`를 건드리지 않고 계획만
  ...
  - **선택 단위는 값 한 줄**(필드), 기본은 전체 선택. 보여주기만 하고 "전부 아니면 전무"면 목록을
    ...
    - **실제 선택 상태를 갖는 건 `.rf-f-cb`(값 한 줄) 하나뿐이다.** 항목(`.rf-item-cb`)·
      필드별(`#rfFields`의 `.rf-fall`)·전체(`#rfAll`) 체크박스는 **그걸 켜고 끄는 손잡이**이고,
      자기 상태는 `syncRefreshPicked`가 매번 되계산한다(따로 들고 있으면 반드시 어긋난다).
      부분 선택은 `indeterminate`로 — 켬/끔 둘 중 하나로 보이면 거짓말이 된다.
    - **필드별 일괄**(`#rfFields`): "평점은 다 받고 시리즈는 두자"가 한 번에 된다.
      실제로 바뀌는 값이 있는 필드만 알약으로 그린다.
    ...
    - **접기/펴기**(`setRefreshFold`, `#rfFoldAll` + 항목별 `.rf-fold`): 목록이 길 때 제목만 훑는다.
      접으면 어떤 필드가 걸렸는지 `.rf-sum`에 한 줄로 남는다(접었다고 정보가 사라지면 안 된다).
      제목을 눌러도 접힌다 — 작은 화살표만 노리기 어렵다.
