...
- **[TMDB 새로고침]**(`refreshTmdbInDetail`, **상세 모달** 하단, TMDB 연결된 기록에만 보임):
  ...
  - ⚠⚠ **TMDB는 영화와 TV가 완전히 다른 id 공간이다** — `movie/12345`와 `tv/12345`는 남남이다.
    ...
    → `sameWork(i, d)`로 **받아온 게 이 기록과 같은 작품인지 확인**하고, 아니면 반대쪽 타입도 보고,
      그래도 아니면 **아무것도 저장하지 않는다.** 성공하면 `i.mediaType`에 남겨 다음부터 짐작을 없앤다.
    → 같은 짐작이 `runRefreshOtts`·`runRefreshRatings`·`runFixNames`·`mediaTypeOf`에도 있다.
      그쪽은 OTT·평점처럼 바꾸는 범위가 좁지만 원인은 같으니, 손볼 때 `mediaType`을 함께 쓸 것. 끝나면 `openDetail`로
