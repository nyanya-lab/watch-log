...
- **시즌 묶기(표시 전용)**: 같은 작품의 시즌들을 카드 하나로 묶어 보여줌. 저장 데이터는 그대로.
  - 묶는 키(`groupKeyOf`) 1순위 = **TMDB 컬렉션**(`collectionId`) — 제목이 달라도 같은 시리즈면 묶임
    (해리포터·반지의제왕). 설정 "시리즈 정보 가져오기"(`runRefreshCollections`)로 채움.
  - 2순위 = `tmdbId` + `title`. 제목까지 봐야 하는 이유: 자동 매칭 때 속편이
