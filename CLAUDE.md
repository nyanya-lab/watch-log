...
```
index.html      640줄  전체 마크업 (헤더/목록/탐색/통계/설정 + 모달 6개)
style.css       607줄  커스텀 CSS (Tailwind CDN 위에 얹음)
core.js         849줄  동기화(REST), 상태(State), 탭, Escape, 유틸
tmdb.js         700줄  TMDB 검색/상세/OTT/추천·발굴/컬렉션 캐시
watchlog.js    2022줄  카드 목록, 필터, 등록·수정 모달, 별점 몰아넣기, 설정 탭
...
- **확인에 실패하면 아무것도 바꾸지 않고 건너뛴다.** 일괄이라 하나씩 물어볼 수 없고, 그냥 두면 기존
  값이 남으므로 잃는 게 없다. 끝나면 **건너뛴 제목 목록**을 보여준다(개수만으로는 뭘 확인할지 모른다).
...
- [ ] watchlog.js가 2000줄 (카드·필터·모달·설정이 한 파일). 쪼개면 편해지지만 구조 변경이라 확인 필요.
