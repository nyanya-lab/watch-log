...
```
index.html      670줄  전체 마크업 (헤더/목록/탐색/통계/설정 + 모달 9개)
style.css       638줄  커스텀 CSS (Tailwind CDN 위에 얹음)
core.js         853줄  동기화(REST), 상태(State), 탭, Escape, 유틸
tmdb.js         700줄  TMDB 검색/상세/OTT/추천·발굴/컬렉션 캐시
watchlog.js    2142줄  카드 목록, 필터, 등록·수정 모달, 별점 몰아넣기, 설정 탭
...
- **조회 → 미리보기 → 적용** 세 단계다(2026-08-07). 조회 단계는 `State`를 건드리지 않고 계획만
  ...
  - **항목마다 체크박스, 기본은 전체 선택**(`#rfAll` + `.rf-cb`). 보여주기만 하고 "전부 아니면 전무"면
    목록을 보여주는 의미가 없다 — 걸리는 것만 체크를 풀면 그 항목은 **그대로 남는다**.
    카드 전체가 `<label>`이라 아무 데나 눌러도 토글된다(폰에서 네모만 노리기 어렵다).
    푼 항목은 흐려지고(`:has(.rf-cb:not(:checked))`), 버튼이 `적용 (N개)`로 바뀐다(`syncRefreshPicked`).
    선택 단위는 **항목**이다 — 필드별로 쪼개면 고를 게 너무 잘아진다.
...
- [ ] watchlog.js가 2100줄 (카드·필터·모달·설정이 한 파일). 쪼개면 편해지지만 구조 변경이라 확인 필요.
