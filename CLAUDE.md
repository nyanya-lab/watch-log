...
- **시즌 묶기(표시 전용)**: 같은 작품의 시즌들을 카드 하나로 묶어 보여줌. 저장 데이터는 그대로.
  - 묶는 키 = `tmdbId` + `title` (`groupKeyOf`). 제목까지 봐야 하는 이유: 자동 매칭 때 속편이
    1편과 같은 tmdbId를 물고 온 경우가 있어(반지의 제왕 3부작 전부 tmdbId 122) tmdbId만으론 오합침.
  - `groupItems(list)` → `{key, main, items}[]`. 정렬은 `applyFilters`에서 끝난 뒤 묶어서 순서 유지.
    `State.groups`에 저장하고 `renderCards`/페이징은 그룹 기준.
  - 묶인 카드: 좌상단에 "시즌 N"(`.wl-season-multi`), 날짜는 시즌 전체를 아우르는 범위.
  - 상세: `seasonsOf()`로 시즌번호 순 정렬 → 시즌별 블록(`.wl-season-row`)에 날짜·횟수·하트·한줄평·수정 버튼.
    단일 시즌이면 기존 레이아웃 그대로.
- 카드 본문: 제목 옆 구분(보라) → 장르(최대 3개, `.wl-genres`) → 날짜. 국가·OTT는 상세에서만.
- 카드 포스터 우상단 세로 스택(`.wl-tr`): TMDB평점(`.wl-vote`) + 그 아래 내 별점 하트(`.wl-myrate`).
  좌상단은 시즌(`.wl-season`). 내 별점=하트(`hearts()`), 상세 모달에도 하트로 표시.
...
- 장르: `visibleGenres()`가 ①`GENRE_KO`로 한글화(TMDB가 TV 장르 일부를 영어로 줌:
    `Action & Adventure`→액션/모험, `Sci-Fi & Fantasy`→SF/판타지, `War & Politics`→전쟁/정치)
    ②구분(type)과 겹치는 장르("드라마" 등) 제거 ③중복 제거. 저장 데이터는 그대로 두고 표시할 때만 변환.
...
요약 타일 6개(총 작품(시즌 묶음) / 시청 기록(시즌별) / 올해 시청 / 재시청 / 평균 별점 / 예상 시청시간).
차트들은 낱개 기록 기준으로 집계(시즌별로 장르·OTT가 같으므로 의도된 동작).
연도별 막대 / GitHub 잔디 히트맵(연도 선택) / 월별 막대(전체) / 장르 도넛 / 구분 도넛 /
국가 가로막대 / OTT 가로막대 / 배우 TOP10 / 감독 TOP10 / 별점 분포(하트).

히트맵은 `startDate~endDate`와 `lastWatchStart~lastWatchEnd` 범위를 날짜 단위로 펼쳐서 집계.

장르 차트는 **가로 막대**(도넛 아님). 한 작품이 장르를 여러 개 가져서 합계가 작품 수를 넘기 때문에
(268작품 → 장르 태그 687개) 도넛의 "전체 중 비율"이 성립하지 않음. 차트 위에 안내문구 표시.
...
- [ ] **속편이 1편과 같은 `tmdbId`를 가진 항목들** — 자동 매칭 오류. 포스터·장르·평점이 1편 것으로
      들어가 있음. 확인된 것: 반지의 제왕 3부작(122), 트와일라잇 브레이킹던(8966), 아바타 불과 재(83533).
      각 항목을 수정 모달에서 올바른 작품으로 다시 검색해 주면 해결됨.
