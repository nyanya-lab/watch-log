...
- 폰트 **마루부리**(네이버, 명조). 2026-07-31 Pretendard(고딕)에서 변경 — "전체가 너무 고딕"이라서.
  - 한글 활자에서 '부리'가 세리프를 뜻한다. 마루부리는 **화면 읽기용으로 만든 명조**라
    나눔명조·본명조보다 작은 크기에서 덜 거칠다.
  - 네이버 배포 CSS는 **굵기마다 폰트 이름이 다르다**(`MaruBuriBold` 등) → `font-weight`가 안 먹는다.
    그래서 그 CSS를 쓰지 않고 **style.css에서 `@font-face`로 한 이름에 다시 묶었다**.
  - 굵기당 ~430KB라 **실제로 쓰는 3개만**(400/600/700) 싣는다. 얇은 굵기는 규칙상 안 쓴다.
  - Pretendard는 폴백으로 남겨둠(index.html 링크 유지).
  - **작은 글씨 대응**: 명조는 11px쯤에서 가로획이 사라져 부서져 보인다. **폰트를 바꿔서 풀 문제가 아니다**
    — 나눔명조·본명조는 인쇄용이라 획 대비가 더 커서 오히려 심하다. 대신 작은 글씨 쪽을 조정했다:
    `.wl-meta` 11→12.5px·500→600·색 진하게, `.badge` 11→12px·600→700,
    `.stat-label`/`.stat-note` 12→13px, 포스터 위 칩 10→11px. 카드 제목도 13→14px
    (명조는 같은 px에서 고딕보다 작아 보인다).
    → **작은 글씨를 더 줄이지 말 것.** 줄이면 이 문제가 바로 돌아온다.
