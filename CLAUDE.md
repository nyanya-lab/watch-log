...
- 내 별점=하트 하나+숫자(`hearts()`, 예 ♥4.5). **5점 만점 소수 입력**(`#fRating` number, `readRating()`).
...
- 소수점 그대로 받는다(4.5). 5 초과는 5로 깎고, 0·빈값·숫자 아님은 저장하지 않는다.
...
- 카드 화면엔 본 날짜·장르·TMDB 평점을 함께 띄운다 (뭘 봤는지 떠올리는 데 필요).
