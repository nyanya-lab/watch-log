...
- OTT 입력: 스트리밍 목록은 TMDB가 자동으로 채우므로(`otts`) 직접 기록하는 건 **영화관 체크박스 + 기타 자유입력**뿐.
  - ⚠ 이 자유입력칸(`#fOtt`)에 **기본값 "넷플릭스"가 박혀 있었다**(~2026-08-03). 드롭다운으로 고르던
    ...
    지금은 빈칸이 기본. **이미 그렇게 저장된 기록은 손대지 않기로 했다** — `ott`가 "넷플릭스"인데
    `otts`에 없는 기록에는 "진짜 넷플릭스에서 봤지만 지금은 내려간 작품"도 섞여 있어 구분이 안 된다.
