...
- `watchlog_rating10` — 별점을 10점 만점으로 바꾸면서 예전 값을 비웠는지 (`"1"`).
  `clearOldRatings` in core.js가 **`syncOnBoot` 다음에** 한 번만 실행한다 — 먼저 지우면 서버에서
  온 별점이 그대로 살아남고, 플래그가 없으면 새로 매긴 별점도 새로고침마다 지워진다
