...
- **상세 모달은 열 때마다 자동으로** 받아 `#dtWatch`에 붙인다(`loadWatchInfo`). 상세는 한 번에
  하나만 열리므로 호출도 1건(~0.3초)이라 버튼을 거칠 이유가 없다. 늦게 온 응답이 다음 작품 화면을
  덮어쓰지 않게 `_watchReq` 순번으로 거른다.
- **탐색은 버튼**(`dcFindOtt`) — 카드(결과는 카드 안 `.dc-watch`)와 미리보기 모달
  (`dcModalFindOtt` → 결과는 `#dcWatch`) 두 곳. 카드가 60장이라 한꺼번에 부르면 ~19초라서 버튼이다.
  `dcFindOtt`은 `btn.closest(".dc-card")` 유무로 카드/모달을 가려 결과를 그릴 자리를 찾는다.
  모달 쪽은 제목을 `Discover._detail`에서 꺼낸다(onclick 속성에 넣으면 따옴표 든 제목에서 깨진다).
  세 경로 모두 같은 `watchInfoHtml` 조각을 쓴다.
...
- **이어보기**: TV의 안 본 시즌 + 영화 시리즈의 안 본 편을 **한 목록에 섞어서** 보여준다.
  ...
  - 영화(`movieContinueList`): `totalSeasons`는 **TV에만 있는 필드**라 영화엔 못 쓴다.
    ...
    - **본 편 번호를 배지에 보여준다**(`seenNos`, 예 `본 편 S1·S3`). 예전엔 "3편 봄"처럼 개수만
      ...
      TV 카드의 `본 시즌 S3` 배지와 같은 형식. `missing`의 정확한 여집합으로 계산한다.
...
- [x] ~~추천·검색 카드에는 OTT 배지 없음~~ (작품마다 별도 호출이 필요해 60개면 ~19초)
      → **[OTT 찾기] 버튼**으로 해결 (2026-08-03). 목록을 그릴 때 다 부르는 대신
      궁금한 카드만 눌러 1건씩 받는다. 아래 "볼 수 있는 곳 찾기" 참고.
