...
const Discover = {
  ...
  /* `RECO_TARGET`개 중 한국 작품 몫. **뽑을 때** 쓰는 값이라 위 필터(보는 조건)와 성격이 다르다.
     입력은 퍼센트(`recoKoPct`)로 받고, 쿼터 로직은 개수(`recoKo`)로 돈다. 기본 70% = 한국 35 · 외국 15.
     ⚠ 아래 개수는 자리만 채워두는 값이다 — 실제로는 `applyDcPrefs`가 퍼센트로 다시 계산한다
...
async function runReco() {
  ...
  try {
    ...
    /* ③ 한국어 작품 발굴 — ①②만으로는 한국 작품이 후보에 거의 안 남는다.
       ...
       한국 후보가 최대 32개인데 외국은 유사작 120 + 장르 64라, 자리를 점수순으로만 채우면
    ...
    /* **같은 영화 시리즈는 한 편만 담는다 — 안 본 편 중 가장 빠른 편**(2026-09-21 요청).
       유사작 추천(①)은 시드의 속편을 잔뜩 물어와서 한 시리즈가 추천 목록의 칸을 여러 개 차지했다.
    ...
    /* 볼 수 있는 곳을 카드마다 조회해 **국내에서 볼 수 있는 것만 추천에 담는다**(2026-08-07).
       예전엔 상위 N개를 그대로 담고 "국내에 없는 것만/볼 수 있는 것만"을 필터로 골랐는데,
       추천은 "이제 뭘 볼까"에 답하는 자리라 **지금 못 보는 작품은 답이 되지 않는다**.

       TMDB는 스트리밍 정보를 목록 응답에 안 주고 작품별 `/watch/providers`에만 주므로
       한 건씩 물어봐야 한다(240ms 간격). 그래서 `RECO_TARGET`개를 채우거나 조회 상한에 닿으면 멈춘다 —
    ...
    /* 한쪽이 몫을 못 채웠으면(안 본 한국 작품이 그만큼 없는 경우) **남는 자리는 다른 쪽으로 메운다.**
       비워두면 `RECO_TARGET`개를 채우라는 목적 자체를 못 지킨다. */
...
const RECO_TARGET = 50;   // 한 번에 담는 추천 개수 (2026-09-21에 60 → 50. 비중은 퍼센트라 자동으로 따라온다)
...
/* 입력칸 옆에 실제 개수를 적는다 — 퍼센트만 보면 전체 중 몇 개인지 머릿속으로 계산해야 한다.
